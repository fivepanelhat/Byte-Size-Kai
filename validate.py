...
async def test_ollama(config):
 ...
 if is_healthy:
 logger.info(
 f"[OK] Point-to-point connection to Ollama at {config.ollama.host} established."
 )
...
async def test_mqtt(config):
 ...
 if is_healthy:
 logger.info("[OK] MQTT connection PASSED")
 logger.info(
...
async def test_av_capture(config):
 ...
 logger.info(f" Video stream: {'[OK]' if video_ok else '[X]'}")

 # Try to start audio stream
 audio_ok = await av_capture.start_audio_stream()
 logger.info(f" Audio stream: {'[OK]' if audio_ok else '[X]'}")

 if video_ok or audio_ok:
 logger.info(
 "[OK] AV capture test PASSED (at least one stream working)"
 )
...
async def test_hardware_control(config):
 ...
 if is_healthy:
 logger.info("[OK] Hardware control test PASSED")
...
async def test_media_pruner(config):
 ...
 logger.info("[OK] Media pruner test PASSED")
...
async def test_ai_agent_methods(config):
 ...
 logger.info("[OK] AI Agent methods test PASSED")
 return True, None
...
async def main():
 ...
 if not config:
 logger.error("\n[X] Cannot proceed - configuration failed")
 logger.warning("\n" + "=" * 60)
 ...
 if passed == total:
 logger.info("[OK] ALL TESTS PASSED - System is ready for deployment!")
 return True
 else:
 logger.warning(
